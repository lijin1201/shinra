#3 last block CBAM ( last block in 3 layers)
import torch
import torch.nn as nn
from torchvision.models import resnet18, ResNet18_Weights
from torchvision.models.resnet import BasicBlock
import re


# ---------------------
# CBAM Modules
# ---------------------
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        super().__init__()
        self.shared_mlp = nn.Sequential(
            nn.Linear(in_planes, in_planes // ratio, bias=False),
            nn.ReLU(),
            nn.Linear(in_planes // ratio, in_planes, bias=False)
        )
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        b, c, _, _ = x.size()
        avg_out = self.shared_mlp(self.avg_pool(x).view(b, c))
        max_out = self.shared_mlp(self.max_pool(x).view(b, c))
        out = avg_out + max_out
        scale = self.sigmoid(out).view(b, c, 1, 1)
        return x * scale


class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        super().__init__()
        padding = kernel_size // 2
        self.conv = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, return_attention=False):
        avg_out = torch.mean(x, dim=1, keepdim=True)
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        x_cat = torch.cat([avg_out, max_out], dim=1)
        attn = self.sigmoid(self.conv(x_cat))
        if return_attention:
            return x * attn, attn
        return x * attn


class CBAM(nn.Module):
    def __init__(self, channels, reduction=16, kernel_size=7):
        super().__init__()
        self.ca = ChannelAttention(channels, reduction)
        self.sa = SpatialAttention(kernel_size)

    def forward(self, x):
        x = self.ca(x)
        x = self.sa(x)
        return x


class CBAMWithMaskSupervision(CBAM):
    def forward(self, x):
        x = self.ca(x)
        x, attn_map = self.sa(x, return_attention=True)
        return x, attn_map


# ---------------------
# Basic Blocks
# ---------------------
class CBAMBasicBlock(BasicBlock):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.cbam = CBAM(self.conv2.out_channels)

    def forward(self, x):
        identity = x

        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)

        out = self.conv2(out)
        out = self.bn2(out)

        out = self.cbam(out)

        if self.downsample is not None:
            identity = self.downsample(x)

        out += identity
        out = self.relu(out)

        return out


class CBAMBasicBlockWithMask(BasicBlock):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.cbam = CBAMWithMaskSupervision(self.conv2.out_channels)

    def forward(self, x):
        identity = x

        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)

        out = self.conv2(out)
        out = self.bn2(out)

        out, attn_map = self.cbam(out)

        if self.downsample is not None:
            identity = self.downsample(x)

        out += identity
        out = self.relu(out)

        return out, attn_map


# ---------------------
# Helper Function
# ---------------------
def _replace_block_layer(layer, block_type, return_attention=False):
    new_layers = []
    for orig_block in layer:
        block_cls = block_type
        block = block_cls(orig_block.conv1.in_channels,
                          orig_block.conv2.out_channels,
                          stride=orig_block.stride,
                          downsample=orig_block.downsample)
        new_layers.append(block)
    return nn.Sequential(*new_layers)

def _replace_block_layer_last(layer, block_type, return_attention=False):
    new_layers = []
    for i, orig_block in enumerate(layer):
        if i == len(layer) - 1:
            block_cls = block_type
            block = block_cls(orig_block.conv1.in_channels,
                            orig_block.conv2.out_channels,
                            stride=orig_block.stride,
                            downsample=orig_block.downsample)
            new_layers.append(block)
        else:
            new_layers.append(orig_block)
    return nn.Sequential(*new_layers)

# ---------------------
# Full Model
# ---------------------
class ResNet18_lbCM(nn.Module):
    def __init__(self, pretrained=True, attr = None):
        super().__init__()
        base = resnet18(weights=ResNet18_Weights.IMAGENET1K_V1 if pretrained else None)
        levels = re.findall(r'LA(\d+)_BL(\d+)',attr)
        self.level = int(levels[0][0]) if len(levels) else -1
        self.btype = int(levels[0][1]) if len(levels) else -1
        if self.btype==0: 
            self.level==-1
        # levels = re.findall(r'CBAM(\d+)',attr)
        # self.cbam_level = int(levels[0]) if levels else -1
        # self.attr = attr

        self.conv1 = base.conv1
        self.bn1 = base.bn1
        self.relu = base.relu
        self.maxpool = base.maxpool

        
    
        self.layer1 = _replace_block_layer_last(base.layer1,
                                                CBAMBasicBlockWithMask if self.btype>1 else CBAMBasicBlock)\
                                                      if self.level==1 else base.layer1
        self.layer2 = _replace_block_layer_last(base.layer2,
                                                CBAMBasicBlockWithMask if self.btype>1 else CBAMBasicBlock)\
                                                      if self.level==2 else base.layer2
        self.layer3 = _replace_block_layer_last(base.layer3, 
                                                CBAMBasicBlockWithMask if self.btype>1 else CBAMBasicBlock)\
                                                      if self.level==3 else base.layer3
        self.layer4 = _replace_block_layer_last(base.layer4, 
                                                CBAMBasicBlockWithMask if self.btype>1 else CBAMBasicBlock)\
                                                      if self.level==4 else base.layer4

        self.avgpool = base.avgpool
        self.fc = base.fc

        if pretrained:
            self.load_state_dict(base.state_dict(), strict=False)

    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)

        if self.level == 1 and self.btype>1:
            x, attn_map = self.layer1(x) 
        else:
            x = self.layer1(x) 

        if self.level == 2 and self.btype>1:
            x, attn_map = self.layer2(x) 
        else:
            x = self.layer2(x) 

        if self.level == 3 and self.btype>1:
            x, attn_map = self.layer3(x) 
        else:
            x = self.layer3(x) 

        if self.level == 4 and self.btype>1:
            x, attn_map = self.layer4(x) 
        else:
            x = self.layer4(x)

        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        x = self.fc(x)

        if self.btype> 1 and self.training:
            return x, attn_map
        else:
            return x
