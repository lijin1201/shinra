...
class MakeDataSet:
    ...
    def cal_mal_factor(self,nodule):
        mal_factor = mean([a.malignancy for a in nodule])
        if mal_factor >3 : mal = 1
        else: mal = 0
        return mal_factor, mal
    ...
    def save_meta(self,meta_list):
        ...
        #pid,nid,sid,pixel_info,cent,cbox,mal_factor,mal
            ...
            if len(nodules_annotation) > 0:
                ...
                # This current for loop iterates over total number of nodules in a single patient
                    ...
                    for nodule_slice in range(mask.shape[2]):
                     ...
                     #nodule_info.append([pid,scan.series_instance_uid,nodule_idx,*scan.spacings,*cbbox[:,0].*cbbox[:,1]], *self.cal_mal_factor(nodule))
            else:
...
if __name__ == '__main__':
    ...
    LIDC_IDRI_list= [f for f in os.listdir(DICOM_DIR) if not f.startswith(('.','LICENSE')) ]
    LIDC_IDRI_list.sort()


    test= MakeDataSet(LIDC_IDRI_list[:3],IMAGE_DIR,MASK_DIR,CLEAN_DIR_IMAGE,CLEAN_DIR_MASK,META_DIR,mask_threshold,padding,confidence_level)
